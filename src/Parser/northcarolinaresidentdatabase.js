...
(async () => {
    try {
        ...
        await page.goto(`https://www.northcarolinaresidentdatabase.com/name/${firstname}-${lastname}`);
        ...
        const results = await page.evaluate(() => {
            ...
            profileList.forEach((profile) => {
                ...
                const age = Array.from(profile.querySelectorAll('.row p'))
                    .find((elem) => elem.textContent.includes('Age'))
                    .textContent.replace('Age: ', '').trim();
                let address = Array.from(profile.querySelectorAll('.row p'))
                    .find((elem) => elem.textContent.includes('Lives at')) || null;
                if (address) {
                    address = address.textContent.replace('Lives at: ', '').trim();
                }
